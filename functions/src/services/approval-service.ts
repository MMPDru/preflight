...
export class ApprovalService {
    ...
    async createApprovalChain(
        ...
        const approvalStages: ApprovalStage[] = stages.map((stage, index) => ({
            ...stage,
            id: `stage-${index + 1}`,
    ...
    async submitApproval(
        ...
        const chain = chainDoc.data() as ApprovalChain;
        ...
        const approvalCount = currentStage.approvals.filter(
            a => a.decision === 'approved' || a.decision === 'partial'
        ).length;

        const rejectionCount = currentStage.approvals.filter(
     ...
     */
    async getPendingApprovals(userId: string): Promise<ApprovalChain[]> {
        ...
        snapshot.forEach(doc => {
            const chain = doc.data() as ApprovalChain;
            const currentStage = chain.stages[chain.currentStageIndex];
    ...
    async getApprovalChain(chainId: string): Promise<ApprovalChain | null> {
        ...
        return doc.exists ? (doc.data() as ApprovalChain) : null;
    }

    /**
     * Get approval chain by job ID
     */
    async getApprovalChainByJobId(jobId: string): Promise<ApprovalChain | null> {
        const snapshot = await this.db
            .collection('approval-chains')
            .where('jobId', '==', jobId)
            .limit(1)
        ...
        return snapshot.docs[0].data() as ApprovalChain;
    }
     ...
     */
    private async handleDeadlineExpired(chainId: string, stageIndex: number): Promise<void> {
        // Log event
        await this.logEvent({
            id: this.db.collection('workflow-events').doc().id,
     ...
     */
    private async notifyStageApprovers(chainId: string, stageIndex: number): Promise<void> {
     ...
     */
    async getApprovalHistory(jobId: string): Promise<WorkflowEvent[]> {
        const snapshot = await this.db
