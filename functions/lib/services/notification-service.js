...
class NotificationService {
     ...
     */
    async sendProofReadyNotification(userId, jobId, jobName) {
        await this.sendNotification(userId, 'proof-ready', 'Proof Ready for Review', `Your proof "${jobName}" is ready for review.`, {
            data: { jobId, jobName },
     ...
     */
    async sendApprovalRequestNotification(userId, chainId, jobName, deadline) {
        const message = deadline
            ? `Please review and approve "${jobName}" by ${deadline.toLocaleDateString()}.`
            : `Please review and approve "${jobName}".`;
        await this.sendNotification(userId, 'approval-request', 'Approval Required', message, {
     ...
     */
    async sendDeadlineReminder(userId, chainId, jobName, hoursRemaining) {
        await this.sendNotification(userId, 'reminder', 'Approval Deadline Approaching', `The approval deadline for "${jobName}" is in ${hoursRemaining} hours.`, {
     ...
     */
    async sendIssueDetectedNotification(userId, jobId, jobName, issueCount) {
        await this.sendNotification(userId, 'issue-detected', 'Issues Detected in PDF', `${issueCount} issue(s) detected in "${jobName}". Please review.`, {
